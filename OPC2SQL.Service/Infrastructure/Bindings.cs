    ...
    public class Bindings : NinjectModule
    {
        public override void Load()
        {
            ...
            Bind<ILog>().ToMethod(x => log4net.LogManager.GetLogger(System.Reflection.MethodBase.GetCurrentMethod().DeclaringType))
                .InSingletonScope();
            Bind<IO2SConfigReader>().To<O2SConfigReader>().InSingletonScope().WithConstructorArgument("filename", filename);
            Bind<IO2SOPCReader>().To<O2SOPCReader>().InSingletonScope();
            Bind<IO2SSQLSender>().To<O2SSQLSender>().InSingletonScope();
        }
    }
}
